...
#define BUFFER_SIZE 1024*1024  //Acutal Transmit buffer size usually set to 1MB
//#define RX_BUFFER_SIZE 1024
#define RX_BUFFER_SIZE 1024*100
...
int main(int argc, char const* argv[])
{
    ...
    char rx_buff_eth[RX_BUFFER_SIZE];
    char rx_buff_wigi[RX_BUFFER_SIZE];
    ...
    struct timeval tv;
	double totalbytes, bytes=0, wifibytes=0, wigibytes=0, ethbytes=0, throughput=0;
...
// Forcefully attaching socket to the port 8080
	...
	while(1) {
	memset(rx_buff_wifi,0,RX_BUFFER_SIZE);
	memset(rx_buff_eth,0,RX_BUFFER_SIZE);
	memset(rx_buff_wigi,0,RX_BUFFER_SIZE);
...
//	printf("\nfwd_ptr:%p and rev_ptr:%p and diff:%ld", fwd_ptr, rev_ptr, rev_ptr-fwd_ptr);fflush(NULL);
	...
	while(1) {
		...
		if(valread > 0 ) {
			//printf("wifi val read:%d", valread);fflush(NULL);
			memcpy(&seqno, (int *)&rx_buff_wifi[0], sizeof(int));
...
//			if(totalbytes>1040000)
       ...
       //valread = recv(ethsocket, rx_buff_eth, RX_BUFFER_SIZE, MSG_DONTWAIT);
        valread = recv(ethsocket, rx_buff_eth, RX_BUFFER_SIZE, MSG_DONTWAIT);
        if(valread > 0 ) {
			//printf("eth val read:%d", valread);fflush(NULL);
			reverse_memcpy(&seqno, (int *)&rx_buff_eth[valread/4], sizeof(int));
            ...
            reverse_memcpy(sec_rev_ptr,rx_buff_eth, valread);
...
//          if(totalbytes>1040000)
		...
		//valread = recv(wigisocket, rx_buff_wigi, RX_BUFFER_SIZE, MSG_DONTWAIT);
		valread = recv(wigisocket, rx_buff_wigi, RX_BUFFER_SIZE, MSG_DONTWAIT);
		if(valread > 0 ) {
			//printf("wigig val read:%d", valread);fflush(NULL);
			reverse_memcpy(&seqno, (int *)&rx_buff_wigi[valread/4], sizeof(int));
			...
			rev_ptr = rev_ptr - valread/4;
			reverse_memcpy(rev_ptr,rx_buff_wigi, valread);
...
/*		if(fwd_ptr >= sec_rev_ptr && sec_rev_ptr >= rev_ptr)
        ...
        if((end_time - start_time) >= 1) {
            ...
            printf("\n%fMBits/sec Received....%.1f", throughput, bytes*8);fflush(NULL);
...
//		totalbytes=0;
        ...
        end_time = tv.tv_sec;
        if((end_time - start_time) >= 1) {
            printf("\n%fMBits/sec Received.....totalbytes:%.1f wifi bytes:%.1f ethbytes:%.1f wigigbytes:%.1f", (bytes*8)/(1000*1000), bytes*8, wifibytes*8, ethbytes*8, wigibytes*8);fflush(NULL);
            fflush(NULL);
            gettimeofday(&tv, NULL);
            start_time = tv.tv_sec;
            bytes=0;
