...
typedef struct fbdt_container_t {
...
{
	int wifiserver_fd, wifiservercmd_fd, wigiserver_fd, wifisocket, wifisocketcmd, wigisocket, valread, sockwifi=0, client_fd1;
	int trialNo=0, wifi_seqno, wigig_seqno;
	...
	FILE *fpt;
	int filesize, lfilesize, ret1;
	fbdt_container wifi_fbdt_container, wigig_fbdt_container;
     fbdt_ack send_fbdt_ack;
...
//Connect to the server...
	...
	serv_addr1.sin_family = AF_INET;
        serv_addr1.sin_port = htons(PORTACK);

        // Convert IPv4 and IPv6 addresses from text to binary
        // form
        if (inet_pton(AF_INET, "192.168.10.231", &serv_addr1.sin_addr)
                <= 0) {
                printf(
...
//#endif
	...
	//memcpy(&filesize, rx_buff, sizeof(int));
	lfilesize=filesize;
		...
		//printf("ongoing filesize:%d", filesize);fflush(NULL);
			...
			if(filesize <= RX_BUFFER_SIZE) {
				...
				wigig_seqno=RX_BUFFER_SIZE;
                        	...
                        	rev_ptr = (int *) (&buff[filesize-RX_BUFFER_SIZE-8]);
				fwd_ptr=(int *)&buff[0];
				wigig_seqno=filesize;
...
#ifdef WIFI
			//valread = recv(wifisocket, rx_buff_wifi, RX_BUFFER_SIZE, MSG_DONTWAIT);
			valread = recv(wifisocket, &wifi_fbdt_container, sizeof(fbdt_container), MSG_DONTWAIT);
			if(valread > 0) {
				printf("val read:%d", valread);fflush(NULL);
			}
			if(valread == sizeof(fbdt_container)) {
				printf("value read:%d", valread);fflush(NULL);
				//if(wifi_fbdt_container.wigig_seqno <= wigig_seqno || wifi_fbdt_container.wifi_seqno >= wifi_seqno) {
				if(wifi_fbdt_container.wifi_seqno >= wifi_seqno) {
					//printf("\nWIFIREAD:wifi_seqno:%d and wigig_seqno:%d", wifi_fbdt_container.wifi_seqno, wifi_fbdt_container.wigig_seqno);fflush(NULL);
					printf("\nWIFIREAD:wifi_seqno:%d ", wifi_fbdt_container.wifi_seqno);fflush(NULL);
					//if(wifi_fbdt_container.wifi_seqno >= wifi_seqno)
						wifi_seqno=wifi_fbdt_container.wifi_seqno+(valread-8);
					//if(wifi_fbdt_container.wigig_seqno <= wigig_seqno)
				//		wigig_seqno=wifi_fbdt_container.wigig_seqno;
					//memcpy(fwd_ptr,rx_buff_wifi, valread);
...
//			if(totalbytes>1040000)
			...
			//valread = recv(wigisocket, rx_buff_wigig, RX_BUFFER_SIZE, MSG_DONTWAIT);
			valread = recv(wigisocket, &wigig_fbdt_container, sizeof(fbdt_container), MSG_DONTWAIT);
			...
			if(valread ==sizeof(fbdt_container)) {
				printf("value read:%d", valread);fflush(NULL);
				//if(wigig_fbdt_container.wigig_seqno <= wigig_seqno || wigig_fbdt_container.wifi_seqno >= wifi_seqno) {
				if(wigig_fbdt_container.wigig_seqno <= wigig_seqno) {
					printf("\nWIGIGREAD:wigig_seqno:%d", wigig_fbdt_container.wigig_seqno);fflush(NULL);
					//if(wigig_fbdt_container.wifi_seqno >= wifi_seqno)
					//	wifi_seqno=wigig_fbdt_container.wifi_seqno;
					//if(wigig_fbdt_container.wigig_seqno <= wigig_seqno)
						wigig_seqno=wigig_fbdt_container.wigig_seqno-(valread-8);
					rev_ptr = rev_ptr - valread/4;
				//memcpy(rev_ptr,rx_buff, valread);
					//reverse_memcpy(rev_ptr,rx_buff_wigig, valread);
...
#endif
			if(wifi_seqno != fwd_ack || wigig_seqno != rev_ack) {
				send_fbdt_ack.fwd_ack=wifi_seqno;	
				send_fbdt_ack.rev_ack=wigig_seqno;
				//memcpy(send_buff, &send_fbdt_ack, sizeof(fbdt_ack));
               			ret1=send(sockwifi, &send_fbdt_ack, sizeof(fbdt_ack), MSG_DONTWAIT);
				if(ret1>0) {
					fwd_ack=wifi_seqno;
					rev_ack=wigig_seqno;
					printf("\nsending ack wifi_seqno:%d and wigig_seqno:%d", wifi_seqno, wigig_seqno);fflush(NULL);
				}
			}
...
//			printf("\nTotalbytes:%.4f wifibytes:%.1f and wigibtes:%.1f", totalbytes, wifibytes, wigibytes);fflush(NULL);
	...
	trialNo++;
	printf("Trial No....%d", trialNo);fflush(NULL);
	//sleep(1);
	}
